...
const VerifiedBadge = ({ userId }: { userId?: string }) => {
  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      const [{ data: pf }, { data: sub }] = await Promise.all([
        supabase.from('premium_features').select('is_active').eq('user_id', userId).eq('is_active', true).gt('expires_at', new Date().toISOString()).maybeSingle(),
